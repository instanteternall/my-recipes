...
import { getRecipeById, updateRecipe, getRecipeCategories, getRecipeTags } from '@/api/recipes'
...
const removeStep = (index: number) => {
  ...
  form.steps.forEach((s: any, i: number) => { s.stepOrder = i + 1 })
}
...
const loadRecipe = async () => {
  ...
  try {
    ...
    form.difficulty = recipe.difficulty || '简单'
    form.categoryId = recipe.categoryId
    form.tagIds = recipe.tags?.map((t: any) => t.id) || []
    form.ingredients = recipe.ingredients?.length
      ? recipe.ingredients
      : [{ ingredientName: '', quantity: 1, unit: '个' }]
    form.steps = recipe.steps?.length
      ? recipe.steps
      : [{ stepOrder: 1, content: '', image: '' }]
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
...
onMounted(() => { loadRecipe(); loadCategories(); loadTags() })
</script>
